...
const LeadsPage: React.FC = () => {
  ...
  const getStatusColor = (status?: string) => {
    const colors: Record<string, string> = {
      ...
      "Booked": "bg-pink-500/20 text-pink-400",
  ...
  return (
    <div className="p-4 md:p-6 text-white bg-background min-h-screen w-full">
      ...
      {error && (
        <div className="mb-4 p-3 bg-red-500/20 border border-red-500 rounded text-red-400">{error}</div>
      )}
          ...
          leads.map((lead) => (
            ...
            >
              ...
              <div className="flex gap-2 pt-3 border-t border-gray-700">
                ...
                <Button variant="ghost" size="sm" className="flex-1 text-red-400 hover:text-red-300" onClick={(e) => { e.stopPropagation(); handleDelete(lead._id, getFullName(lead)); }}>
                ...
                <tr key={lead._id} className="border-t border-gray-700 hover:bg-gray-800 cursor-pointer" onClick={() => navigate(`/leads/${lead._id}`)}>
                  ...
                  <td className="p-3">
                    <div className="flex gap-2">
                      ...
                      <Button variant="ghost" size="sm" className="text-red-400 hover:text-red-300" onClick={(e) => { e.stopPropagation(); handleDelete(lead._id, getFullName(lead)); }}>
      ...
      {totalPages > 1 && (
        <div className="flex flex-col sm:flex-row justify-center md:justify-end items-center gap-3 mt-4">
          <div className="flex items-center gap-3">
            <Button variant="secondary" size="sm" disabled={page === 1} onClick={() => setPage((p) => p - 1)} className="flex-1 sm:flex-none">
              <ChevronLeft className="h-4 w-4 mr-1" /><span className="hidden sm:inline">Prev</span>
            </Button>
            <span className="text-sm text-gray-400 px-2">{page} / {totalPages}</span>
            <Button variant="secondary" size="sm" disabled={page === totalPages} onClick={() => setPage((p) => p + 1)} className="flex-1 sm:flex-none">
